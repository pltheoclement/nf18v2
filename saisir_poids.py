...
def saisir_poids(HOST, DATABASE, USER, PASSWORD):
    ...
    while (not erreur):
        try:
            mesure = int(input("Mesure du poids (en g) : "))
            erreur = True
        except:
            print ("Erreur, veuillez entrer un entier positif")
    ...
    try:
        sql = "INSERT INTO Poids VALUES (%i, '%i-%i-%i', %i)"% (mesure, ASaisie, MSaisie, JSaisie, numDossier)
        cur.execute(sql)
        conn.commit()
